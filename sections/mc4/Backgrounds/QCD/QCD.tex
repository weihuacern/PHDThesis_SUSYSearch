...
The low \MET scaled $T_{QCD}^{Data}$ are calculated by real data and MC samples. The \ttbar, $W$+jets and single top MC events are reweight with a scale factor, which value is 84\%. The scale factor is derived from the data/MC comparison in the single-muon control samples, with ISR reweighing and the fake b scale factor. The Z+jets event yields are calculated in the same way decribed in the last section. The TTZ and Rare events are directly from MC samples, with generator-level leptonically decaying W and Z bosons veto.
