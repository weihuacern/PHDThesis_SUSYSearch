The lepton veto does not succeed in rejecting events when light leptons (electrons or muons) are not isolated, not identified/reconstructed, or are out of the acceptance region. The expected number of these residual events passing the search selection criteria in $35.9$~fb$^{-1}$ is presented in Tab.\ref{tab:nexpLL}. The prediction of the lost lepton background is performed using a data-driven method described below.
...
%The $m_{\rm T}$ distribution in \ttbar events and for a T2tt signal point 
%($m_{\sTop}$=850 GeV, $m_{LSP}=100$ GeV) is shown in Fig.~\ref{fig:LLMTW}. 
...
the forward region with an out-of-acceptance $|\eta|$. %, or when electrons are emitted in the transition reigion.
...
%as illustrated in Fig.~\ref{fig:2lep}, which shows the distribution of the
%number of jets with $p_{T}>$30 GeV in \ttbar MC events with one (black) 
%or with one or two (red) lost leptons.
...
%Figures~\ref{fig:ClosureTestMCmuons},~\ref{fig:ClosureTestMCmuons2},~\ref{fig:ClosureTestMCele} and~\ref{fig:ClosureTestMCele2} show 
%this closure test for muons only and electrons only, respectivelly, for
%various search variables. 

Fig.~\ref{fig:closureSBisotrk} shows the results of this closure test.

%The number of predicted and expected events (muons plus electrons) 
%are shown in Fig.~\ref{fig:closureSBisotrk} for all search bins.%, while
%%Fig.~\ref{fig:closureSBnoisotrk} shows these numbers 
%%before applying the isolated track veto.
%%The closure results for inverted $\Delta\phi$ selection
%%used in the QCD estimation is shown in Fig.~\ref{fig:ClosureTestInvDPhi}.

%%% Closure test muon only
%\begin{figure}[htbp]
%\begin{center}
%\begin{tabular}{c}
%\includegraphics[width=0.7\textwidth]{sections/mc4/Backgrounds/LostLepton/figures/_mu_all_njets.pdf}\\
%\includegraphics[width=0.7\textwidth]{sections/mc4/Backgrounds/LostLepton/figures/_mu_all_met.pdf}\\
%\end{tabular}
%\end{center}
%\caption{Monte Carlo-based closure test of the method (muons only). The lost
%lepton prediction is compared to the (true) MC expectation. 
%The error bars are the statistical uncertainty of the 
%prediction. Systematical uncertainties are not shown.} 
%\label{fig:ClosureTestMCmuons}
%\end{figure}
%\begin{figure}[htbp]
%\begin{center}
%\begin{tabular}{c}
%\includegraphics[width=0.7\textwidth]{sections/mc4/Backgrounds/LostLepton/figures/_mu_all_mt2.pdf}\\
%\includegraphics[width=0.7\textwidth]{sections/mc4/Backgrounds/LostLepton/figures/_mu_all_ntopjets.pdf}
%\end{tabular}
%\end{center}
%\caption{Monte Carlo-based closure test of the method (muons only). The lost
%lepton prediction is compared to the (true) MC expectation. 
%The error bars are the statistical uncertainty of the 
%prediction. Systematical uncertainties are not shown.} 
%\label{fig:ClosureTestMCmuons2}
%\end{figure}

\begin{figure}[htbp]
\begin{center}
\begin{tabular}{c}
%\includegraphics[width=0.85\textwidth, height=0.35\textheight]{sections/mc4/Backgrounds/LostLepton/figures/closure_lept_sb_isotrk.pdf}%\\
...
%The lost lepton method includes the prediction of single top events. Because acceptance and efficiencies are computed for ttbar, the closure checks have been made also with single top Monte Carlo to verify that the single top contribution is well predicted. For the baseline, the prediction is $xx\pm xx$ and the expectation $xx\pm xx$. The closure is within $xx$\% for single top.
...
Both dileptonic corrections are only minor compared to the remaining ones so a conservative systematic uncertainty is assigned here. %About 3\% of the total single-lepton control sample originates from dileptonic \ttbar decays (after applying the \MT cut) and less than 2\% of expected search region events. 
...
%8\% should be assigned for muon tracks, 15\% for electron tracks and 10\% for pion tracks. Furthermore, the statistical uncertainties from MC are propagated.
\item MC closure:\\
Furthermore, an uncertainty on the precision of the MC closure test is assigned. Since no true non-closure is observed (see Fig.~\ref{fig:closureSBisotrk}) the larger value of the non-closure or the stat. uncertainty on non-closure is assigned for each bin. %In case this ratio is dominated by extremely low MC statistics and a high spread in weights the assigned non-closure uncertainty is cut off at 100\%.
\end{itemize}
...
shown in Fig.~\ref{fig:LostLeptonResult_mu} and Fig.~\ref{fig:LostLeptonResult_ele} (red points) and listed in Tab.~\ref{tab:LLpred} to Tab.~\ref{tab:LLpred2b} for both muon and electron channels.% and Tab.~\ref{tab:LLpred37}.
%Table~\ref{tab:LostLeptonResult}.% for inclusive search bins and in Table~\ref{tab:LostLeptonResult2} for exclusive search bins.

Applying the procedure indicated by Eq.~\ref{eq:lostleptonequation}, each event in the control sample is weighted by the various efficiencies. %The distribution of the average weight computed in \ttbar, single top and W$+$jets MC is displayed in Fig.~\ref{fig:TFLL} for each search bin.
Few bins have $0$ predicted events.
In that case the statistical uncertainty is computed as the upper bound of the statistical uncertainty on 0 given by the Garwood interval (1.8) multiplied by the average translation factor to go from a raw number of events to the prediction. %shown in Fig.~\ref{fig:LostLeptonResult} and Tab.~\ref{tab:LLpred} to Tab.~\ref{tab:LLpred2b}.
...
%The breakdown in term of not-isolated, not-reconstructed, and out of acceptance leptons is given in Table~\ref{tab:breakdown}.
%
%\begin{table}[htb]
%\caption{Breakdown of the lost lepton prediction results. Uncertainties are statistical uncertainty only.} 
%\begin{center}
%\begin{tabular}{|c|c|c|c|c|}
%\hline
% & MET$>200$~GeV, 1b & high MET, 1b & MET$>200$~GeV, 2b & high MET, 2b \\
%\hline
%%muon: out of acceptance & $15.6\pm 1.3$ & $1.4\pm 0.4$ & $5.8\pm0.8$ & $0.2\pm0.1$ \\
%%\hline
%%muon: not-reco & $8.8\pm 0.9$ & $1.0\pm 0.3$  & $3.9\pm0.7$ & $0.2\pm0.1$ \\
%%\hline
%%muon: not isolated & $6.0\pm 0.8$ & $0.5\pm 0.2$  & $2.3\pm0.5$ & $0.1\pm0.1$ \\
%%\hline
%%total muon  & $30.4\pm 2.7$ & $2.9\pm 0.8$  & $11.9\pm1.7$ & $0.4\pm0.3$ \\
%%\hline
%%\hline
%%electron: out of acceptance & $14.4\pm 1.2$ & $1.3\pm 0.4$  & $5.3\pm0.7$ & $0.2\pm0.1$ \\
%%\hline
%%electron: not-reco & $43.2\pm 4.9$ & $3.9\pm 1.5$  & $16.0\pm3.0$ & $0.5\pm0.4$ \\
%%\hline
%%electron: not isolated & $1.4\pm 0.2$ & $0.1\pm 0.1$  & $0.6\pm0.1$ & $0.1\pm0.1$ \\
%%\hline
%%total electron  & $59.0\pm 5.9$ & $5.3\pm 1.8$  & $21.8\pm3.6$ & $0.8\pm0.5$ \\
%\hline
%\end{tabular}
%\end{center}
%\label{tab:breakdown}
...
%The final prediction for the baseline is compared in Fig.~\ref{fig:LostLeptonCompare} to plain MC simulation.
%%% pred vs plain MC baseline
%\begin{figure}[hp]
%\begin{center}
%%\includegraphics[width=0.55\textwidth]{lostleptons/figures/c10_baseline.pdf}
%\end{center}
%\caption{MET distribution for the baseline. Black points are the prediction from data. Colored lines are the expectations from MC simulation.}
%\label{fig:LostLeptonCompare}
%\end{figure}
